...
tuner = Hyperband(
    ...
    seed=69,
    directory = 'ML/Tuner',
    project_name = 'Research_project_Hyperband_whole'
    , overwrite=True
)

tuner.search(X_train, y_train, epochs=60
            , callbacks=[EarlyStopping(monitor='val_mse', patience=3, mode='min')]
            , validation_data = (X_test, y_test))

model2 = tuner.get_best_models(num_models=1)[0]

model2.save("ML/Tuner/optimal_model_trained_whole_dataset")

best_hp = tuner.get_best_hyperparameters()[0]
model1 = tuner.hypermodel.build(best_hp)

model1.save("ML/Tuner/optimal_model_untrained_whole_dataset")
...
plt.title("Performance of KerasTuner-optimised model - Whole Dataset")
